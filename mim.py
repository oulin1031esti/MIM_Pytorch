...
###
# File: mim.py
# Created Date: Saturday, April 9th 2022, 4:41:19 pm
# Author: Oulin
# -----
# Last Modified:
# Modified By:
...
class MIM(nn.Module):
    def __init__(self, input_dims, out_dims, in_shape, device, hidden_dim=[32, 32],
            ...
            if i < 1:
                self.stlstm_layer.append(
                    SpatioTemporalLSTMCell(
                        self.input_dims, self.hidden_dim[i],
    ...
    def forward(self, frames, teacher_forcing_ratio=0.0):
        st_memory = None
        ...
        for ts in range(self.total_length - 1):
            if ts < self.input_length:
                ...
                x_gen = frames[:, ts] if use_teacher_forcing else x_gen
            preh = hidden_state[0]
            # 1st layer(convlstm)
